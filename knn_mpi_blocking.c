...
int main(int argc, char *argv[]){
    ...
    pstart = MPI_Wtime();                        // Time of program when mpi started
    ...
    for (i=0 ; i<ROWS ; i++){       // First element to compare
        ...
        MPI_Send(&knn[i][0], K, knn_type, (pid + 1) % pnumber, 0, MPI_COMM_WORLD);      // pid sends to pid + 1 (last process sends to MASTER=0)
    ...
    ptime = pend - pstart;           // Program's time
